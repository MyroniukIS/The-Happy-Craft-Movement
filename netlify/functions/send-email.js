...
exports.handler = async function (event, context) {
  try {
    ...
    if (!firstName) {
      return { statusCode: 400, body: JSON.stringify({ field: 'firstName', error: 'First name is required.' }) };
    }
    if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      return { statusCode: 400, body: JSON.stringify({ field: 'email', error: 'A valid email is required.' }) };
    }
    if (!message) {
      return { statusCode: 400, body: JSON.stringify({ field: 'message', error: 'Message is required.' }) };
    }
    ...
    if (!SMTP_HOST || !SMTP_PORT || !SMTP_USER || !SMTP_PASS) {
      ...
      return { statusCode: 500, body: JSON.stringify({ error: 'Mail service not configured on server.' }) };
    }
    ...
    const debug = process.env.DEBUG_MAIL === 'true' || process.env.NODE_ENV !== 'production';
    const body = { error: 'Failed to send message.' };
    if (debug) body.detail = err && err.message ? String(err.message) : String(err);
